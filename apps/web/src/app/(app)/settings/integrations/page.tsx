...
import { AlertTriangle, CheckCircle2, MoreHorizontal, Plus, Radio, Send, Webhook } from "lucide-react";
...
export default function IntegrationsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Integrations"
        description="Stream every audit event, in order and without gaps, to your SIEM or your own systems."
        actions={
      ...
      {dests.isPending ? (
        ...
        <Card>
          <EmptyState icon={<Radio />} title="Nothing is streaming yet" description="Send sign-ins, admin changes, device and policy events to Splunk, Datadog or a webhook. Outages are retried; nothing is lost." />
        </Card>
      )}
      ...
      <div className="flex flex-wrap items-start gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-fg-muted">
            {KINDS[d.kind].label} · {d.format === "ocsf" ? "OCSF" : "Nexus JSON"} · {d.event_filter.length ? d.event_filter.join(", ") : "all events"} · <span className="font-mono">{d.url}</span>
          </p>
...
function AddDestination({ onClose, onSecret }: { onClose: () => void; onSecret: (s: string) => void }) {
  ...
  const pick = (k: Dest["kind"]) => {
    ...
    setFormat(k === "webhook" ? "nexus" : "ocsf");
  };
        ...
        unwrap(
          api.POST("/v1/event-destinations", {
            body: {
              ...
              config: kind === "splunk_hec" ? (index ? { index } : {}) : kind === "datadog" ? (tags ? { tags } : {}) : {},
      ...
      <DialogContent title="Add a destination" className="max-w-lg">
        <div className="max-h-[70vh] space-y-4 overflow-y-auto pr-1">
          <div className="grid grid-cols-3 gap-2">
            {(Object.keys(KINDS) as Dest["kind"][]).map((k) => (
          ...
          {kind === "datadog" ? (
            ...
            <Field label={kind === "splunk_hec" ? "HEC URL" : "Endpoint URL"} htmlFor="d-url" hint={kind === "splunk_hec" ? "e.g. https://splunk.example.com:8088/services/collector/event" : "Must be https"}>
              <Input id="d-url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://" />
            </Field>
          )}
          {kind !== "webhook" ? (
            <Field label={kind === "splunk_hec" ? "HEC token" : "API key"} htmlFor="d-secret">
          ...
          <div className="flex justify-end gap-2 border-t border-border pt-4">
            ...
            <Button variant="primary" loading={create.isPending} disabled={!name.trim() || !finalUrl.trim() || (kind !== "webhook" && !secret)} onClick={() => create.mutate()}>
              Add destination
            </Button>
          </div>
        </div>
