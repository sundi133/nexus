...
const Provider = z.enum(["google", "entra"]);
...
async function listOut(tx: Tx): Promise<z.infer<typeof Connection>[]> {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      name: r.name,
      account: r.provider === "google" ? cfg.admin_email ?? "" : cfg.tenant_id ?? "",
      enabled: r.enabled,
      ...
      next_sync_at: r.enabled ? new Date((r.last_sync_at?.getTime() ?? Date.now()) + (r.last_sync_at ? r.interval_minutes * 60_000 : 0)).toISOString() : null,
...
export function registerDirectorySyncRoutes(app: App) {
  ...
  app.openapi(
    createRoute({
      ...
      security: bearer,
      request: { ...idParam, ...body(SettingsIn.partial().extend({ name: z.string().trim().min(1).max(100).optional(), credentials: Creds.optional() })) },
      responses: list,
    }),
    async (c) => {
      const p = requirePermission(c, "directory:sync");
