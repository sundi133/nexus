...
export function layout(opts: { heading: string; body: string; items?: { title: string; meta?: string; url: string }[]; cta: { label: string; url: string }; footer: string }) {
  const items = opts.items?.length
    ? `<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;border-top:1px solid #e6e7ea">${opts.items
        .map(
          (i) =>
            `<tr><td style="padding:10px 0;border-bottom:1px solid #e6e7ea"><a href="${esc(i.url)}" style="color:#0f1115;text-decoration:none;font-size:14px;font-weight:500">${esc(i.title)}</a>${i.meta ? `<br><span style="font-size:12px;color:#8a909b">${esc(i.meta)}</span>` : ""}</td></tr>`,
        )
        .join("")}</table>`
    : "";
...
${items}<a href="${esc(opts.cta.url)}" style="display:inline-block;background:#4f46e5;color:#fff;text-decoration:none;padding:10px 18px;border-radius:6px;font-size:14px;font-weight:600">${esc(opts.cta.label)}</a>
<p style="margin:24px 0 0;font-size:12px;color:#8a909b">${esc(opts.footer)}</p>
</td></tr></table></td></tr></table></body></html>`;
  const list = opts.items?.length ? `${opts.items.map((i) => `- ${i.title}${i.meta ? ` (${i.meta})` : ""}\n  ${i.url}`).join("\n")}\n\n` : "";
  const text = `${opts.heading}\n\n${opts.body}\n\n${list}${opts.cta.label}: ${opts.cta.url}\n\n${opts.footer}`;
  return { html, text };
}
