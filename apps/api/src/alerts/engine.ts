...
export const BUILTIN_RULES: RuleDef[] = [
  ...
  { key: "ai_network_tool", name: "AI tool ran a network tool", description: "An AI assistant or agent on a device (Cursor, Claude, Codex…) ran curl, scp, ssh or a similar tool: it may be moving data off the device. See the device's process events.", severity: "high", match: { types: ["device.detection"], details: { detection: "ai_network_tool" } }, group_by: "target", threshold: 1, window_minutes: 60 },
  { key: "risk_raised", name: "Someone's risk became high", description: "A person's access-graph risk rose to high or critical: see the reasons on the Access graph page.", severity: "high", match: { types: ["user.risk_raised"] }, group_by: "target", threshold: 1, window_minutes: 1440 },
