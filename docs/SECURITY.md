...
- **Access requests:** nobody decides their own request. The owner role can't be requested, and admin roles are always time-limited. Approving an admin role needs a recent MFA, as does self-activation for pre-approved people. Grants end by themselves and roles are re-read on every request, so a lapsed grant stops working at once. The audit log links each grant to its request and approvals.
- **Device actions:** agent commands are signed with an Ed25519 key per organization. The agent pins that key at enrollment (or on its first check-in if it enrolled earlier) and refuses any other key. It runs a command only if it's for this device, not expired and not run before. Lock, restart and wipe need step-up and a reason. Wipe goes through the MDM only, needs the device's name typed and the `devices:wipe` permission (owners and admins; never API keys), and alerts every admin.
- **MDM connections** are read-only (Intune: DeviceManagementManagedDevices.Read.All; Jamf: Read Computers). Their client secrets are sealed, and a Jamf URL has to pass the same outbound-URL checks as webhooks.
