...
| ADR-032 | Access requests grant the same objects admins create (an app assignment, a group membership, a user_roles row) and remember them on the request, which is the unit of lifecycle: pending → active → ended/revoked. Approval stages are resolved at decision time (manager, group, users, role), falling back to owners and admins when a stage has nobody; a minute scheduler ends expired grants in every tenant (2026-09-25) | A separate entitlement model; PIM-style eligible roles stored on the user; resolving approvers at request time | Nothing downstream (SSO, SCIM provisioning, RBAC) needs to know about requests, and a grant's end is just undoing its one row. Resolving approvers late means a changed manager or group takes effect for pending requests. |
| ADR-033 | An access review snapshots grants at launch into items (user × grant: direct app assignment, group membership — for via-group app access too — or role) and applies decisions only at close, per item, re-checking what's still there (already gone, last owner skipped) and routing request-made grants through the request's lifecycle (2026-09-25) | Live queries at close; revoking at decision time | A snapshot is what auditors certify (who had access on a date); applying at close lets reviewers change their minds and gives one reviewed, audited moment of change. |
| ADR-034 | Dynamic group rules are stored as JSON on the group and compiled to parameterised SQL over a fixed attribute list. Membership is materialised in `group_members` (so SSO, provisioning, conditional access and reviews read one table) and re-evaluated by a deduplicated per-org job queued whenever people are touched, plus a 15-minute sweep; changes are diffed, audited and pushed through `touchUsers`/`touchGroups` (2026-09-25) | Evaluating rules at read time everywhere; a rules engine or expression language; per-user evaluation on every change | Nothing downstream needed to learn about rules. A fixed attribute list keeps rules safe (no arbitrary SQL, no secrets) and indexable. One job per org at a time makes a 10,000-person directory sync cost one evaluation, not 10,000. |
| ADR-035 | AI agents are first-class principals, not API keys or service users: each has an accountable owner, credentials (client secret, private_key_jwt, or workload identity federation with exact issuer/subject/audience matching), and gets short-lived (default 15 min) `at+jwt` tokens from the org's existing issuer via `client_credentials`, audience-bound (RFC 8707) to the org's MCP gateway or one server behind it. The kill switch is a `tokens_valid_after` timestamp plus status, read by the gateway on every call; offboarding or containing the owner suspends their agents until reassigned (2026-09-25) | Agents as API keys; agents as users; long-lived tokens with a revocation list | Agents act in production with real data, so they need the same lifecycle rigour as people (owner, offboarding, review) without human MFA. Federation removes the most common leak (a static secret in CI). A per-call status check makes revocation instant without distributing revocation lists. |
| ADR-036 | The MCP gateway terminates the protocol instead of proxying it: agents connect to `{api}/mcp/{org}/{server}` (streamable HTTP, JSON responses, no server-initiated stream) and the gateway answers initialize/ping/tools/list itself, forwarding only authorized `tools/call` to the upstream over its own session with credentials sealed in Nexus. Tools are approved by hash of description + schema + annotations, so any change (tool poisoning, drift) takes a tool out of service until re-approved; risk classes come from MCP annotations, then name heuristics, then admins. Rules are deny-by-default allow/deny on agent, tag or everyone × tool × risk × argument conditions, deny wins; every decision is audited with an argument hash, never argument values (2026-09-25) | A transparent reverse proxy; Cedar from day one; approval by tool name | A transparent proxy would pass resources, prompts, sampling and elicitation we can't yet govern. Hashing the description matters because descriptions are prompts. A small rule model covers the common cases (allowlisted repos, read-only agents, no destructive tools) and maps onto Cedar later without changing callers. |
| ADR-037 | The audit log is a per-tenant hash chain in commit order (txid, id) — the gapless order event streaming already uses — sealed hourly into blocks (range, count, digest before and after). Each seal is itself an audit event, so digests reach the customer's SIEM and archive: the anchor lives outside Nexus. The database refuses UPDATE and DELETE on audit rows (app role has no grant; a trigger stops even the owner) except for retention, which removes whole sealed blocks only once every enabled destination has received them, keeping the chain verifiable (2026-09-25) | Chaining by timestamp; per-event chaining at insert time; a transparency log service | Timestamps are assigned at transaction start, so a long transaction would land in an already-sealed day; txid order with the snapshot xmin never misses one. Insert-time chaining would serialize every write across a tenant. Anchoring through the customer's own SIEM costs nothing and is what auditors can check independently. |
| ADR-038 | Alerting is a rule engine over the audit log, following it with a per-org cursor in commit order (like streaming), not hooks in each feature. A rule is event types + outcome + detail equality, counted per actor, target, IP or overall within a window; an open alert absorbs further matches (one alert, one notification, a growing count). Verdicts on resolution (true/false positive, benign) feed per-rule quality. Paging uses the alert ID as the PagerDuty dedup key / Opsgenie alias, so acknowledgements sync both ways without storing their incident IDs; inbound webhooks authenticate with a per-integration URL token (2026-09-25) | Detection code in each feature; a CEP/SIEM engine; alerting through the notification system only | Every security-relevant fact is already an audit event, so one engine covers every feature, including future ones. Deduplication and quality metrics are what keep alerting low-noise (OPS-08); heavy correlation belongs in the customer's SIEM, which gets the same events. |
| ADR-039 | Config as code is one declarative document over the API (`GET /v1/config`, `POST /v1/config/plan`, `POST /v1/config/apply`) that names things instead of using IDs, with plan and apply sharing one reconcile function (apply = plan + writes, in one transaction, with the plan's hash to refuse stale plans). Omitted sections are untouched; deletes need `prune`. Secrets never leave; the CLI injects them from environment variables. The `nexus` CLI (TypeScript, one bundled file) wraps it with YAML and a confirmation step (2026-09-25) | A Terraform provider first; per-resource CRUD from the CLI; exporting IDs | One reconcile path means the plan reviewers approve is exactly what runs. Names make staging-to-production promotion work. A Terraform provider can later be a thin layer over the same three endpoints. |
| ADR-040 | RBAC v2 adds custom roles (catalog permission sets, never admins:manage) and role grants (a custom role, or Help Desk / Security Analyst / Read-only limited to groups) beside the existing org-wide user_roles. The principal carries resolved grants: permission → everywhere or a set of groups. Scopes apply only to permissions about people and their devices; a scoped grant keeps groups:read and apps:read and drops everything else that would see beyond the scope (e.g. audit:read). Deny by default: a permission held only within groups counts only on routes that opt in with `{ scoped: true }` and then filter or check the subject (2026-09-25) | Scope as a filter applied in the data layer everywhere; Cedar now; separate role tables per feature | Opt-in keeps every existing and future route safe: forgetting to add scope support means scoped admins are refused, never over-granted. User and device routes, where Help Desk works, opt in. Owners still manage all admin access, and grants appear in admin access reviews and are removed by offboarding. |
| ADR-041 | On-prem directories have two paths over one shared reader (`@nexus/ldap-directory`, on ldapts): a direct LDAPS/StartTLS connection from Nexus as a fourth pull provider (same plan, preview, guard and schedule), and the `nexus directory push` connector that runs inside the network and reconciles over the existing inbound SCIM API. Presets for Active Directory (objectGUID, userAccountControl) and OpenLDAP; nested groups are expanded client-side; plain ldap:// without StartTLS is refused; private directory hosts need an explicit deployment flag. Delegated authentication binds as the user (looked up by directory ID) at sign-in, never with an empty password (2026-09-25) | A Go on-prem agent with a persistent tunnel; LDAP-only direct sync; storing AD password hashes | Most hosted customers can't expose AD, and SCIM push already has the guard, links and deprovisioning semantics, so the connector is small and needs no inbound ports. Sharing the reader keeps both paths' mapping identical. Delegated auth over direct connections keeps passwords in AD; hosted customers use federation (ADFS/Entra) for AD passwords. |
