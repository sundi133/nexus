...
func Pack(goos string) []Scheduled {
	...
	switch goos {
	case "darwin":
		software = `SELECT name, bundle_short_version AS version, 'app' AS source, bundle_identifier AS publisher FROM apps
...
FROM listening_ports l LEFT JOIN processes p ON p.pid = l.pid WHERE l.port > 0`, MaxRows: 500},
		...
		{Name: "browser_extensions", SQL: `SELECT e.browser_type AS browser, e.name, e.identifier, e.version, u.username AS user FROM users u CROSS JOIN chrome_extensions e USING (uid)
UNION ALL SELECT 'firefox' AS browser, f.name, f.identifier, f.version, u.username AS user FROM users u CROSS JOIN firefox_addons f USING (uid)`, MaxRows: 2000},
